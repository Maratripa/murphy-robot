...
class Brazo():
    ...
    def __init__(self, length_1: float, length_2: float, precision: float, pencil_diff: float, camera_dist: float,
                  min_q1: float, max_q1: float, min_q2: float, max_q2: float):
    ...
    def get_angles(self, pose_deseada):
        ...
        return inverse_kinematics(2, {"pose_deseada": pose_deseada, "L1": self.L1, "L2": self.L2, "precision": self.precision, "q1": self.q1, "q2": self.q2, "pencil_diff": self.pencil_diff, "camera_dist": self.camera_dist})
...
class Ojos():
    ...
    def pixel_to_mm(self, image):

        wound_center_pixel, wound_countour = find_wound(image)

        wound_countour = wound_countour.reshape(wound_countour.shape[0], 2)
        distance_matrix = squareform(pdist(wound_countour))
        i, j = np.unravel_index(np.argmax(distance_matrix), distance_matrix.shape)

        point1 = np.array((wound_countour[i][0], wound_countour[i][1]))
        point2 = np.array((wound_countour[j][0], wound_countour[j][1]))
        length_axis = point2 - point1
        wound_length_axis_distance = np.linalg.norm(length_axis)
        pix_to_mm = 79 / wound_length_axis_distance
        print("pix_to_mm:", pix_to_mm)
        return pix_to_mm
    ...
    def define_stiching_points(self, image, height: float):
        info = {"pix_to_mm": self.pixel_to_mm(image), 
    ...
    def estimate_stitch_pose(self, stitches: np.array, height: float, actual_pose: np.array):
        ...
        for i in range(stitches.shape[0]):
            ...
            print(mm_diff)
            print(actual_pose)
            stitches_pos[i] = actual_pose - np.array([mm_diff[0], mm_diff[1]+15, height])
...
class Robot():
    ...
    def __init__(self, brazo: Brazo, ojos: Ojos, comunicacion: Comunicacion, precision: float,
                  height: float, Kp_2D: float, min_z: float, max_z: float):
    ...
    def pose(self):
        ...
        "z": self.z,
        "z2": self.comunicacion.send_request("z2", 5),
        "pencil_height": self.pencil_height}
        return pose_calc(3, info)
    
    def get_state(self, pose_deseada):
        return inverse_kinematics(3, {"pose_deseada": pose_deseada, "L1": self.brazo.L1, "L2": self.brazo.L2, "camera_dist": self.brazo.camera_dist, "pencil_height": self.pencil_height, "z": self.z, "pencil_diff": self.brazo.pencil_diff,"z2": self.comunicacion.send_request("z2", 5)})
    ...
    def center_wound(self):
        ...
        print("move vector")
        print(move_vector * 0.2852)
        pix_to_mm = self.ojos.pixel_to_mm(image)
        print(self.pose())
        while distance > self.ojos.precision:
            
            new_pose = self.brazo.pose() - pix_to_mm*move_vector
    ...
    def stitch_pos(self):
        ...
        return True
    def z_touch(self):
        height = self.comunicacion.send_request("z1",5)
        effector_depth = self.comunicacion.send_request("z2",5)
        pencil_height = self.pencil_height #Diferencia entre la punta del lapiz y el extremo del brazo verticalmente
        z = -effector_depth + pencil_height + height
        return z
